...
import { ArrowRight, ArrowLeft, CheckCircle2, Check, Loader2, Lock } from "lucide-react";
...
export default function QuestionnairePage() {
  ...
  const loadAnswers = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/reports/${reportId}/questionnaire`,
        {
  ...
  function handleNext() {
    if (currentSection === 0) {
      // About section — just advance, no save
          ...
          <main className="flex-1 flex items-center justify-center px-6">
            <div className="text-center max-w-md">
              ...
              <p className="text-sm text-ink-mid font-light leading-relaxed mb-6">
                Your answers have been saved to this report. The operations summary
                will be regenerated with the updated data.
              </p>
              <Button render={<a href={`/report/${reportId}`} />}>
                Return to Report
              </Button>
            </div>
          </main>
          ...
          <div className="flex flex-wrap gap-1.5 mb-8">
            {sections.map((sec, idx) => (
              <button
                ...
                className={cn(
                  "flex-shrink-0 px-4 py-2 rounded-full text-sm font-medium transition-all border cursor-pointer",
                  idx === currentSection
